package intalio.cts.mobile.calendarlibrary;
...
import intalio.cts.mobile.calendarlibrary.calendar.CalendarInstance;
...
//
    ...
    public void onClick(View view) {
        ...
        if (!temp.getText().toString().trim().isEmpty()) {
            ...
            if (MyCalendarView.language == Language.Arabic.getLanguageValue()) {
            ...
            for (int j = 1; j <= 7; j++) {
                ...
                if (count-1 < calendarInstance.lengthOfMonthAccordingToyear(calendarInstance.getCurrentYear(),calendarInstance.getCurrentMonth())) {
                    ...
                    if (firstTime && j == calendarInstance.getWeekStartFrom()) {
                        String text = language == Language.Arabic.getLanguageValue() ? CalendarInstance.toArabicNumbers(pretex + count + "") : pretex + count + "";
                        textView.setText(text);
                        firstTime = false;
                        count++;
                    } else if (!firstTime) {
                        textView.setText(language == Language.Arabic.getLanguageValue() ? CalendarInstance.toArabicNumbers(pretex + count + "") : pretex + count + "");
                        count++;
                        isDatePassed = true;
                    } else {
                        textView.setText(" ");
